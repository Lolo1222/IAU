...
def prepare_attack_data(model, data_in, data_out):
    ...
    if torch.is_tensor(y_in):
        ...
        # y_in_one_hots = np.eye(NUM_CLASSES)[y_in.astype("int32")]
        # y_out_one_hots = np.eye(NUM_CLASSES)[y_out.astype("int32")]
    y_hat_in = softmax(model.predict_proba(X_in))
    y_hat_out = softmax(model.predict_proba(X_out))

    # labels = np.ones(y_in_one_hots.shape[0])
